...
describe('submission detail page helpers', () => {
    ...
    it('normalizes queue-origin detail context down to a canonical queue page and strips unrelated params', () => {
        expect(
            resolveSubmissionDetailNavigationContext('queue-1', {
                source: 'queue',
                page: ['4', '9'],
            ...
            source: 'queue',
            queueHref: '/queues/queue-1?page=4',
            resultsHref: null,
        });
        ...
        expect(
            ...
            source: 'queue',
            queueHref: '/queues/queue-1?page=1',
            resultsHref: null,
        });
    });
    ...
    it('keeps queue-origin and results fallback labels and targets queue scoped unless sanitized results context is present', () => {
        ...
        expect(getSubmissionDetailBackHref('queue-1', 'queue')).toBe('/queues/queue-1?page=1');
        expect(
            getSubmissionDetailBackHref('queue-1', 'queue', { queueHref: '/queues/queue-1?page=3' })
        ).toBe('/queues/queue-1?page=3');
    ...
    it('returns queue-origin visits to the canonical queue page preserved in detail context', () => {
        const router = {
            back: mock(() => undefined),
        ...
        expect(router.push).toHaveBeenCalledWith('/queues/queue-1?page=3');
    });
    ...
    it('falls back to the sanitized contextual results URL when a results-origin visit has no history', () => {
        ...
        expect(router.back).not.toHaveBeenCalled();
        expect(router.push).toHaveBeenCalledWith('/queues/queue-1?page=1');
...
describe('SubmissionDetailPageContent', () => {
    it('renders queue-origin loading state with queue-scoped return copy and preserved page-aware breadcrumbs', () => {
