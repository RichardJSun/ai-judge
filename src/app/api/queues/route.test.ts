...
class FakeSupabaseClient {
    ...
    constructor(
        private readonly executors: Record<
            string,
            QueryExecutor<unknown> | QueryExecutor<unknown>[]
        >
    ) { }
...
describe('handleGetQueues', () => {
    ...
    it('returns a reviewer-safe 500 when derived count lookups fail or page metadata is malformed', async () => {
        const countFailureClient = new FakeSupabaseClient({
            queues: () => json([createQueueRow(1)], 1),
            submissions: () => failure('submissions unavailable'),
            question_templates: () => json([]),
        });

        const countFailureResponse = await handleGetQueues(createRequest('http://localhost/api/queues?page=1'), {
            createServiceClient: () => countFailureClient as never,
        });

        expect(countFailureResponse.status).toBe(500);
        expect(await countFailureResponse.json()).toEqual({ error: 'Failed to load queues.' });
